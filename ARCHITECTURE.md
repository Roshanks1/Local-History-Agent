...
2. **Discover** (`article_discovery.py`): use ZIM title and full-text indexes, normalize possessives, filter instruction/pronoun words, and resolve redirects to canonical article paths. Follow-up discovery uses the current topic instead of searching the entire conversation.
3. **Extract/index** (existing `history_ai.build`, `chunking.py`, `structured_wikipedia.py`): read complete candidate articles, including content lists/tables, and reuse cached embeddings. This makes additional sections available without another ingestion system. The unchanged legacy `wikipedia_local.py` preserves the original benchmark corpus.
4. **Retrieve and expand** (`retrieval_expansion.py`): rank initial semantic hits; score articles using similarity, occurrence frequency, and title relevance; retain relevant candidate articles that missed the initial cutoff. Revisit all chunks in the shortlisted articles. Combine vector scores with intent-related section bonuses, cap repeated sections, reserve trigger evidence for causal questions, and merge/deduplicate paragraphs. Cause queries use a second generic intent-expanded embedding query. Comparison retrieval reserves evidence from multiple shortlisted articles. This is a transparent heuristic, not a trained reranker.
5. **Build chronology when needed** (`timeline_utils.py`): extract dated passages with date text, numeric sort key, precision, approximate flag, source article, section, and original chunk ID. Preserve dates and ranges; reject certain incomplete or misleading references. Use an explicit requested range, a relevant article's stated range, or a bounded continuation window when available. Sort and sample dated evidence before generation. Preceding source sentences remain available to resolve source pronouns.
6. **Pack evidence** (`answer_context.py`): honor the character cap and estimated-token budget. Each supplied passage retains traceable source metadata and records truncation. The token estimate is UTF-8 bytes / 3, rounded up, not the exact Qwen tokenizer. Configuration reserves space within the existing 8,192-token model context; actual `prompt_eval_count` is saved when Ollama reports it.
7. **Generate** (`history_ai.generate`): use the original user question, bounded dialogue context, and current source evidence. Prompts distinguish facts/inferences, background/triggers, and parallel comparison criteria. Timeline generation selects source labels and short descriptions using a local JSON schema; code supplies source dates, drops unknown labels, caps ten entries and sorts them. Dates are not taken from model-generated date fields. Event summaries can still be inaccurate and need review.
8. **Remember/display** (`conversation_state.py`, `main.py`, `terminal_output.py`): remember only successful turns, print readable sources and citations, and continue prompting.
...
`retrieval_config.json` contains validated values for candidate/initial/article/context counts, per-article/per-section limits, similarity threshold, section/article boosts, context budgets, history bounds, timeline sizes, and chunk settings. Fields omitted from the JSON use dataclass defaults in `retrieval_config.py`.
...
- Discovery candidates and canonical articles.
- Raw/intent-adjusted initial hits, article scores/frequency, expanded chunks and selected context.
- Similarity/expansion scores, headings, source IDs, date records and bounds.
...
Run `python -m unittest discover -s tests` in the existing environment for regression tests. `evaluate_assistant.py` compares raw and expanded retrieval on the original five provisionally labeled questions; `--live` also exercises the three-turn cause conversation, Defenestration continuation, and a dated Thirty Years' War timeline. Files are saved incrementally so failures are inspectable. The frozen baseline artifacts are not overwritten.
